...
class UpdateFolderAction
{
     ...
     * Update a folder
     */
    public function execute(VideoOverlayFolder $folder, UpdateFolderDTO $dto): VideoOverlayFolder
    {
        try {
            $updates = [];
            ...
            if ($dto->name !== null) {
                ...
                $existing = VideoOverlayFolder::where('user_id', $folder->user_id)
     ...
     */
    private function getDescendantIds(VideoOverlayFolder $folder): array
    {
        $ids = [];
        $children = VideoOverlayFolder::where('parent_id', $folder->id)->get();

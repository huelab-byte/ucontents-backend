...
class UpdateFolderAction
{
     ...
     * Update a folder
     */
    public function execute(FootageFolder $folder, UpdateFolderDTO $dto): FootageFolder
    {
        try {
            $updates = [];
            ...
            if ($dto->name !== null) {
                ...
                $existing = FootageFolder::where('user_id', $folder->user_id)
     ...
     */
    private function getDescendantIds(FootageFolder $folder): array
    {
        $ids = [];
        $children = FootageFolder::where('parent_id', $folder->id)->get();

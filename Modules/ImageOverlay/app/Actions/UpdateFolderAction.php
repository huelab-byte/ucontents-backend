...
class UpdateFolderAction
{
     ...
     * Update a folder
     */
    public function execute(ImageOverlayFolder $folder, UpdateFolderDTO $dto): ImageOverlayFolder
    {
        $updates = [];
        ...
        if ($dto->name !== null) {
            ...
            $exists = ImageOverlayFolder::where('user_id', $folder->user_id)
                ->where('name', $dto->name)
     ...
     */
    private function getDescendantIds(ImageOverlayFolder $folder): array
    {
        $ids = [];
        $children = ImageOverlayFolder::where('parent_id', $folder->id)->get();

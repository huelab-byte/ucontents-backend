...
class UpdateFolderAction
{
     ...
     * Update a folder
     */
    public function execute(AudioFolder $folder, UpdateFolderDTO $dto): AudioFolder
    {
        try {
            $updates = [];
            ...
            if ($dto->name !== null) {
                ...
                $existing = AudioFolder::where('user_id', $folder->user_id)
     ...
     */
    private function getDescendantIds(AudioFolder $folder): array
    {
        $ids = [];
        $children = AudioFolder::where('parent_id', $folder->id)->get();

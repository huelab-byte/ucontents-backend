...
class DeleteFolderAction
{
     ...
     */
    public function execute(ImageFolder $folder, bool $forceDelete = false): array
    {
        ...
        return DB::transaction(function () use ($folder, $folderPath, $forceDelete) {
            ...
            try {
                ...
                // 2. Delete all images in this folder
                $this->deleteFolderImages($folder, $forceDelete, $stats);
     ...
     */
    private function deleteChildFolders(ImageFolder $folder, bool $forceDelete, array &$stats): void
    {
        ...
        foreach ($children as $childFolder) {
            ...
            // Delete images in child folder
            $this->deleteFolderImages($childFolder, $forceDelete, $stats);
     ...
     */
    private function deleteFolderImages(ImageFolder $folder, bool $forceDelete, array &$stats): void
    {
        $imageItems = $folder->images()->get();

        foreach ($imageItems as $image) {
            try {
                $this->deleteImageAction->execute($image, $forceDelete);
                ...
                Log::error('Failed to delete image in folder', [
                    'folder_id' => $folder->id,
                    'image_id' => $image->id,
                    'error' => $e->getMessage(),
                ]);
    ...
    private function deleteStorageDirectory(?string $folderPath, array &$stats): void
    {
        ...
        try {
            $storagePath = 'image-library/' . $folderPath;
